...
test('unauthorized endpoints and duplicate signup are blocked', async (t) => {
  const { server, baseUrl, dbFile } = await startServer();
  t.after(() => {
    server.close();
    fs.rmSync(dbFile, { force: true });
  ...
  const unauthRoom = await fetch(`${baseUrl}/api/v1/rooms`, { method: 'POST' });
  assert.equal(unauthRoom.status, 401);

  await signup(baseUrl, 'dup@example.com');
  const duplicate = await fetch(`${baseUrl}/api/v1/auth/signup`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ name: 'Dup', email: 'dup@example.com', password: 'password1234' })
  });
  assert.equal(duplicate.status, 409);
});

test('room capacity conflict under sequential contention', async (t) => {
  const { server, baseUrl, dbFile } = await startServer();
  t.after(() => {
    server.close();
    fs.rmSync(dbFile, { force: true });
  ...
  const owner = await signup(baseUrl, 'owner@example.com');
  const guest = await signup(baseUrl, 'guest@example.com');
  const ownerHeaders = { 'content-type': 'application/json', authorization: `Bearer ${owner.accessToken}` };
  const guestHeaders = { 'content-type': 'application/json', authorization: `Bearer ${guest.accessToken}` };

  const roomRes = await fetch(`${baseUrl}/api/v1/rooms`, {
    method: 'POST',
    headers: ownerHeaders,
    body: JSON.stringify({ name: 'Cap1', maxParticipants: 1 })
  });
  const room = (await roomRes.json()).data;

  const joinOwner = await fetch(`${baseUrl}/api/v1/rooms/${room.roomId}/join`, { method: 'POST', headers: ownerHeaders });
  assert.equal(joinOwner.status, 409);

  const joinGuest = await fetch(`${baseUrl}/api/v1/rooms/${room.roomId}/join`, { method: 'POST', headers: guestHeaders });
  assert.equal(joinGuest.status, 409);
});

test('openapi contract includes critical paths', () => {
  const openapi = readFileSync(path.resolve(process.cwd(), '../../docs/architecture/openapi.yaml'), 'utf8');
  const requiredPaths = [
    '/api/v1/auth/signup',
    '/api/v1/auth/login',
    '/api/v1/rooms/{roomId}/games',
    '/api/v1/games/{gameId}/skip',
    '/api/v1/analytics/events',
    '/api/v1/analytics/mapping/verify'
  ];
  for (const expected of requiredPaths) {
    assert.equal(openapi.includes(expected), true, `Missing contract path: ${expected}`);
  }
});
