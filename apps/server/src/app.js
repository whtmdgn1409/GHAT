...
export function createApp(options = {}) {
  ...
  return async function handler(req, res) {
    ...
    const roomParticipantsParams = matchPath(pathname, '/api/v1/rooms/:roomId/participants');
    if (method === 'GET' && roomParticipantsParams) {
      const room = repositories.rooms.findById(roomParticipantsParams.roomId);
      if (!room) return fail(res, requestId, 'ROOM_NOT_FOUND', 'Room not found', 404);
      const participants = realtimeHub ? realtimeHub.getRoomPresence(room.roomId) : [];
      return ok(res, requestId, { roomId: room.roomId, participants });
    }
